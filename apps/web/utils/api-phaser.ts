...
// const DataPhaser = (responseData: unknown) => {
//     // Type assertion only once
//     const data = responseData as responseType;

//     // Directly transform data in a single pass
//     const phaseredData = data.map(({ id, order, rowData }) => {
//         const dataSet: Record<string, any> = { id, order };

//         rowData.forEach(({ name, content }) => {
//             // Parse content only if it's a string
//             dataSet[name] = typeof content === 'string' ? JSON.parse(content) : content;
//         });
